...
def remove_django_cms_apps():
    shutil.rmtree(
        os.path.join("{{cookiecutter.project_slug}}", "apps", "news-djangocms")
    )
...
def main():
    ...
    if "{{ cookiecutter.cms }}" == "Wagtail":
        ...
        if {{cookiecutter.include_news_app}}:
            os.rename(
                os.path.join(apps_dir, "news-wagtail"), os.path.join(apps_dir, "news")
            )
    if "{{ cookiecutter.cms }}" == "DjangoCMS":
        ...
        if {{cookiecutter.include_news_app}}:
            os.rename(
                os.path.join(apps_dir, "news-djangocms"), os.path.join(apps_dir, "news")
            )
    ...
    if {{cookiecutter.add_submodule_fn}}:
        os.system(
            f"git submodule add https://github.com/stephband/Fn.git {os.path.join(static_path, 'fn')}"
        )
    if {{cookiecutter.add_submodule_dom}}:
        os.system(
            f"git submodule add https://github.com/stephband/dom.git {os.path.join(static_path, 'dom')}"
        )
    if {{cookiecutter.add_submodule_slideshow}}:
        os.system(
            f"git submodule add https://github.com/stephband/slide-show.git {os.path.join(static_path, 'slide-show')}"
        )
    settings_dir = os.path.join(
        "{{cookiecutter.project_slug}}", "{{cookiecutter.project_slug}}", "settings"
    )
    shutil.move(
        os.path.join(settings_dir, "local.py-template"),
        os.path.join(settings_dir, "local.py"),
    )
    ...
    if os.environ.get("GITHUB_WORKFLOW"):
        ...
        exit_code = os.system("python3 -m venv .venv")
    if exit_code:
    ...
    if not os.environ.get("GITHUB_WORKFLOW"):
        ...
        if exit_code:
            print(
                "Project generation failed. Cannot cd in {{cookiecutter.project_slug}}"
            )
            return exit_code
        exit_code = os.system("createdb {{cookiecutter.project_slug}}")
        if exit_code:
            print(
                "Project generation failed. Cannot create db with name {{cookiecutter.project_slug}}"
            )
            return exit_code
        if exit_code:
            ...
            return exit_code
        exit_code = os.system(
            ".venv/bin/python {{cookiecutter.project_slug}}/manage.py migrate"
        )
        if exit_code:
        ...
        if {{cookiecutter.cms == "Wagtail"}}:
            exit_code = os.system(
                "echo \"Site.objects.all().update(hostname='{{cookiecutter.domain_name}}', site_name='{{cookiecutter.project_name}}')\" | .venv/bin/python {{cookiecutter.project_slug}}/manage.py shell_plus"
            )
        else:
            exit_code = os.system(
                "echo \"Site.objects.all().update(domain='{{cookiecutter.domain_name}}', name='{{cookiecutter.project_name}}')\" | .venv/bin/python {{cookiecutter.project_slug}}/manage.py shell_plus"
            )
        if exit_code:
            print("Project generation failed. Cannot set domain name in shell_plus")
            return exit_code
        exit_code = os.system(
            "echo \"User.objects.create_superuser('info@cruncher.ch','admin' )\" | .venv/bin/python {{cookiecutter.project_slug}}/manage.py shell_plus"
        )
